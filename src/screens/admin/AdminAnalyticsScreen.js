...
import { ChevronLeft, TrendingUp, TrendingDown, DollarSign, Users, Ticket, Activity, Download } from 'lucide-react-native';
...
const AdminAnalyticsScreen = ({ navigation }) => {
  ...
  const StatCard = ({ label, value, change, type, icon, iconBg }) => (
    <View style={styles.statCard}>
      <View style={styles.statHeader}>
        <View style={[styles.iconContainer, { backgroundColor: iconBg || 'rgba(29, 53, 87, 0.05)' }]}>
           {icon}
        </View>
        <View style={[styles.changeBadge, { backgroundColor: type === 'up' ? 'rgba(5, 150, 105, 0.1)' : 'rgba(230, 57, 70, 0.1)' }]}>
          {type === 'up' ? <TrendingUp size={12} color="#059669" /> : <TrendingDown size={12} color={COLORS.error} />}
          <Text style={[styles.changeText, { color: type === 'up' ? '#059669' : COLORS.error }]}>{change}</Text>
        </View>
      </View>
            ...
            <View style={styles.chartContainer}>
              {revenueByMonth.map((item, index) => (
                <View key={index} style={styles.barContainer}>
                  <View style={[styles.bar, { height: (item.value / maxRevenue) * 150, backgroundColor: index % 2 === 0 ? '#1d3557' : COLORS.error }]} />
          ...
          <View style={styles.chartCard}>
            ...
            <View style={{marginTop: 20}}>
                ...
                <View key={index} style={styles.progressRow}>
                    ...
                    <View style={[styles.progressBarFill, { width: `${(item.count / 5000) * 100}%`, backgroundColor: index === 0 ? COLORS.error : '#1d3557' }]} />
                    </View>
                </View>
                ))}
            </View>
