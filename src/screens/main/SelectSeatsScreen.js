...
import { ChevronLeft, Info, Sparkles, Plus, Minus, Crosshair } from 'lucide-react-native';
...
const SelectSeatsScreen = ({ navigation }) => {
  const renderSectorBlock = (sector) => {
    ...
    let sectorStyle = {};
    if (sector.id === 'north') sectorStyle = { top: '10%', alignSelf: 'center', backgroundColor: '#3b82f6', width: 120, height: 80, borderRadius: 20 };
    if (sector.id === 'south') sectorStyle = { bottom: '20%', alignSelf: 'center', backgroundColor: '#ef4444', width: 120, height: 80, borderRadius: 20 };
    if (sector.id === 'vip') sectorStyle = { top: '40%', right: '10%', backgroundColor: '#fbbf24', width: 80, height: 60, borderRadius: 16 };

    return (
      <TouchableOpacity 
        key={sector.id} 
        style={[styles.sectorBlock, sectorStyle]} 
        onPress={() => navigation.navigate('SeatBlock', { sector })}
        activeOpacity={0.8}
      >
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerTitle}>
          <Text style={styles.title}>Select Zone</Text>
      ...
      <View style={styles.mapArea}>
        ...
        <View style={styles.zoomControls}>
           <TouchableOpacity style={styles.zoomButton}><Plus size={20} color={COLORS.text} /></TouchableOpacity>
           <TouchableOpacity style={styles.zoomButton}><Minus size={20} color={COLORS.text} /></TouchableOpacity>
           <TouchableOpacity style={[styles.zoomButton, {marginTop: 8}]}><Crosshair size={20} color={COLORS.text} /></TouchableOpacity>
        </View>
      </View>
    </View>
  );
