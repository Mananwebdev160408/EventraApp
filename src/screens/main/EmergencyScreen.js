...
const EmergencyScreen = ({ navigation, route }) => {
  ...
  // Get event/stadium context from navigation params if available
  const eventId = route?.params?.eventId || null;
  const stadiumId = route?.params?.stadiumId || null;
  const seatInfo = route?.params?.seatInfo || null;
  ...
  const handleSosPress = async () => {
    ...
    try {
      ...
      const response = await sosService.raiseSos(sosData);
      setIsSosActive(true);
      // Try to extract SOS ID from response for later resolution
      if (response?.sosId) setActiveSosId(response.sosId);
      else if (response?.id) setActiveSosId(response.id);
    } catch (error) {
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.sosContainer}>
            ...
            {/* Main Button */}
            <TouchableOpacity activeOpacity={0.8} onPress={handleSosPress}>
              <Animated.View
                style={[
                  styles.sosButton,
          ...
          <View style={styles.statusContainer}>
            ...
            <View style={styles.locationCard}>
              ...
              <Text style={styles.locationValue}>
                {seatInfo || "Location being shared via GPS"}
              </Text>
            </View>

            <Text style={styles.disclaimer}>
              A security team member and onsite medic are being dispatched to
              your exact location. Please stay where you are.
            </Text>
          </View>
        </View>
